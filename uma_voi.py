...
@sv.on_fullmatch("草女士", "那我呢", "今天的小特不是我的对手", "草上飞", "啊草", "那我呢", "阿草")
async def voisend(bot,ev) -> MessageSegment:
    try:
...
@sv.on_fullmatch("数码", "cdd", "dd", "DD", "CDD", "爱丽数码")
async def voisend(bot,ev) -> MessageSegment:
    try:
...
@sv.on_fullmatch("茶座", "插座", "小仓唯", "xcw", "曼哈顿咖啡", "xcw", "曼城茶座")
async def voisend(bot,ev) -> MessageSegment:
    try:
...
@sv.on_fullmatch("大米洗澡", "小祖宗","米宝", "米浴")
async def voisend(bot,ev) -> MessageSegment:
    try:
